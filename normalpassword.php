...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Fetch superuser's current password (MD5 encrypted) from the database
    $query = $conn->prepare("SELECT password FROM superuser WHERE username = ?");
    ...
    if (!$storedPassword) {
        ...
        $updateQuery = $conn->prepare("UPDATE superuser SET password = ? WHERE username = ?");
        $updateQuery->bind_param("ss", $hashedPassword, $superUsername);
...
<head>
    ...
    <title>Change Normaluser Password</title>
    <style>
       body {
    ...
    <script>
        ...
        function checkPasswordMatch() {
            ...
            matchMessage.style.display = newPassword !== confirmPassword ? "block" : "none";
        }
    </script>
</head>
...
<body onload="<?php echo ($successMsg || $errorMsg) ? "showMessage('" . ($successMsg ? 'success' : 'error') . "', '" . ($successMsg ? $successMsg : $errorMsg) . "')" : ''; ?>">
    <div class="container">
        ...
        <div id="msgDiv"></div>
        <form action="normalpassword.php" method="POST">
